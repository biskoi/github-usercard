...
function cardMaker(attr){
  ...
  axios.get(followersLink).then(reply => {
    cardFollowers.textContent = `Followers: ${reply.data.length}`;
  });

  axios.get(followingLink).then(reply => {
    cardFollowing.textContent = `Following: ${reply.data.length}`;
  });
...
// let followersArray = ['sonicemma', 'MMGroesbeck', 'alan5123', 'fjhansen', 'MelodyRackham'];
let followersArray = [];
